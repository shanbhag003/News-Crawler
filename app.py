...
if input_operation == "Search News":
    ...
    if st.button("Fetch"):
        if input_sub_operation == "See the headlines":
            ...
            article = pd.DataFrame(summ, columns=['Article'])
            ...
            # Clean the headlines
            SUMMARY_OUTPUT['Article'] = SUMMARY_OUTPUT['Article'].apply(cleantext)
            ...
            st.write(SUMMARY_OUTPUT.to_html(escape=False, index=False), unsafe_allow_html=True)
                ...
                # Clean the headlines
                SENTIMENT_OUTPUT['Article'] = SENTIMENT_OUTPUT['Article'].apply(cleantext)
