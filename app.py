    ...
    if st.button("Fetch"):
        if input_sub_operation == "See the headlines":
            ...
            st.write(TOP_OUTPUT.to_html(escape=False, index=False), unsafe_allow_html=True)
                ...
                SENTIMENT_OUTPUT = pd.concat([headlines, links, source], axis=1)  # Normal
                ...
                # Clean the headlines
                SENTIMENT_OUTPUT['Headlines'] = SENTIMENT_OUTPUT['Headlines'].apply(cleantext)
                ...
                SENTIMENT_OUTPUT['Sentiment'] = SENTIMENT_OUTPUT['Headlines'].apply(getpolarity)
